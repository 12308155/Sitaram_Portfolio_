            ...
            if (entry.isIntersecting) {
                ...
                if (img.dataset.src) {
                    const onLoad = () => {
                        ...
                        const mockup = img.closest('.project-preview-mockup');
                        if (mockup) mockup.classList.add('has-image-loaded');
                        img.removeEventListener('load', onLoad);
                    };

                    const onError = () => {
                        const mockup = img.closest('.project-preview-mockup');
                        if (mockup) mockup.classList.remove('has-image-loaded');
                        img.remove();
...
// ── Magnetic Tilt & Spotlight Effect for Interactive Cards ────────────────
...
const revealItems = document.querySelectorAll('.reveal-in-up, .reveal-heading, .stagger-item, .card-hover-effect');
const revealObserver = new IntersectionObserver((entries) => {
    entries.forEach((entry, index) => {
        if (entry.isIntersecting) {
            // Apply a slight delay if it has the stagger-item class
